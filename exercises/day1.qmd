...
Let's first read and observe the dataset. This dataset is located in`data` directory inside the project:

```{r}
#| label: vaccine
vaccine_administrated <- ---("data/vaccine_adminstrated.csv")
vaccine_administrated
```

After importing your data, it's important to double-check the structure of the data and the data types of the variables. The structure of the data you import into R must be a `tibble`. 

:::{.callout-note}
## Question
Verify the data type of each variable in `vaccine_administered`. We may need to modify the data type of `month`, `strike`, `flooding_events`. Could you explain why? 
:::

```{r}
library(lubridate)
vaccine_administrated <- vaccine_administrated |> 
  mutate(month = ---(month), strike = ---(strike), flooding_events = ---(flooding_events))

vaccine_administrated <- vaccine_administrated |> 
  mutate(month = ym(month), strike = as_factor(strike), flooding_events = as_factor(flooding_events))
```
...
:::{.callout-caution}
The dataet you use may contain missing values (`NA` in R) and other data quality issues. While checking and fixing data quality issues are crucial for any project, addressing such issues goes beyond the scope of this training. For further information, you may refer to additional resources [Link].
:::
...
Write a r code to check for duplicate observations in the data.
...
:::{.callout-note}
## Question
Have you identified any duplicate observations? 
:::

If your data contains no duplicates, you may proceed with creating a tsibble. Otherwise, you need to fix it. 
...
Which functions do you use to check for temporal gaps? complete the following R chunk:
...
Is there any temporal gap in the data?
...
How would you create quarterly time series of dose administrated in each region?
...
What if you need to create quarterly dose administrated in the country, Write the R to achieve that.
...
Dp you observe any pattern that has not been obvious with time plot and season plot?

### ACF plots

In forecasting, we would be interested in understanding how similar a time series is to the lags of itself. We often measure this similarity by calculating the correlation (i.e. the linear asscociation) between a time series and its lags and then plot it. 
...
What can you say about the correlation between dose administrated in each month?
