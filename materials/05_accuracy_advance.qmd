...
6. Use residual diagnostic based on residuals in the training set to see wether all informations is captured by models.
7. Create different windows to evaluate forecast accuracy using time series cross validation
8. Train model to each window
5. To view the results of accuracy, use the accuracy() function with the `fable` as the first argument and original data  as the second.
6. Pick a measure in the output to evaluate the forecast(s); a smaller error indicates higher accuracy.
7. Forecast using all data for test set and visualise forecasts against actual values
8. Finally, produce forecast using the selected approach for future.
