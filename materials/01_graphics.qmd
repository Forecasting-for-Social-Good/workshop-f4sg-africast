...
#| cache: false
source("setup.R")
# ggtime provides the time series graphics (gg_season(), gg_subseries(),
# gg_lag(), and autoplot() for tsibbles/ACF); feasts is reserved for
...
```{r ggtime_calendar, eval=FALSE}
library(ggtime)
library(mixtime)
vic_elec |>
  filter(lubridate::year(Date) == 2014) |>
  as_tibble() |>
  mutate(Time = datetime(Time)) |>
...
* `coord_calendar()` cuts a time axis into a calendar-like grid of rows/columns
* each row above is a month, and each cell within it a day

## Calendar plots
\fontsize{10}{11}\sf

```{r ggtime_calendar2, ref.label="ggtime_calendar", echo=FALSE, out.height="90%", fig.height=5.4, fig.width=9}
```
